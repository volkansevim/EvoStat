...
class Gnuplot
{
	...
	/// My addition (VS)
	Gnuplot& savetoeps(const std::string &filename = "gnuplot_output");
    //----------------------------------------------------------------------------------
    // set and unset
...
#elif ( defined(unix) || defined(__unix) || defined(__unix__) ) && defined(__CLUSTER__)
std::string Gnuplot::terminal_std = "postscript";
...
//------------------------------------------------------------------------------
//
// saves a gnuplot session to an eps file
//
Gnuplot& Gnuplot::savetoeps(const std::string &filename)
{
    cmd("set terminal postscript eps color");

    std::ostringstream cmdstr;
    cmdstr << "set output \"" << filename << ".eps\"";
    cmd(cmdstr.str());
                             ...
                             const std::string &title)
{
    ...
    //
    int iIndex = 0;
    for(int iRow = 0; (unsigned int) iRow < iHeight; iRow++)
    {
        for(int iColumn = 0; (unsigned int) iColumn < iWidth; iColumn++)
        {
            tmp << iColumn << " " << iRow  << " " 
